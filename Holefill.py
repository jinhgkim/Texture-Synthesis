    ...
    for y in range(rows):
        for x in range(cols):
            if hole_mask[x, y] == 1:
                if (
                    hole_mask[x - 1, y] == 0
                    or hole_mask[x + 1, y] == 0
                    or hole_mask[x, y - 1] == 0
                    or hole_mask[x, y + 1] == 0
                ):
                    edge_mask[x, y] = 1
    return edge_mask
...
assert (
    (iFillMin >= patchL)
    and (iFillMax < imRows - patchL)
    and (jFillMin >= patchL)
    and (jFillMax < imCols - patchL)
), "Hole is too close to edge of image for this patch size"
...
assert (texImRows > patchSize) and (
    texImCols > patchSize
), "Texture image is smaller than patch size"
...
while nFill > 0:
    ...
    while nTODO > 0:
        ...
        TODOPatch = imHole[
            iPatchCenter - patchL : iPatchCenter + patchL + 1,
        ...
        ssdValue = ssdIm1[
            min(round(abs(random.gauss(0, randomPatchSD))), np.size(ssdIm1) - 1)
        ]
        ...
        selectPatch = textureIm[
            iSelectCenter - patchL : iSelectCenter + patchL + 1,
